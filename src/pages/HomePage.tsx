...
import { Gamepad2, Film, MessageCircle, Music, Sparkles, ArrowRight, Zap, Star, Users, Rocket, Send, Wifi, WifiOff } from 'lucide-react';
...
export function HomePage({ setPage }: { setPage: (p: Page) => void }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { count: rooms } = await supabase.from('game_rooms').select('*', { count: 'exact', head: true }).eq('is_active', true);
      setRoomCount(rooms || 0);
    })();
    const ch = supabase.channel('home-uc')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'profiles' }, () => setUserCount(c => c + 1))
      .on('postgres_changes', { event: '*', schema: 'public', table: 'game_rooms' }, async () => {
        const { count } = await supabase.from('game_rooms').select('*', { count: 'exact', head: true }).eq('is_active', true);
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="max-w-6xl mx-auto px-6 py-10">
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-white">Wszystko w jednym miejscu</h2>
          <p className="text-slate-400 mt-1 text-sm">Czat, gry, muzyka i kino — bez przełączania aplikacji</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {FEATURES.map((f, i) => {
            const Icon = f.icon;
            return (
              <div key={i} className="group relative p-5 rounded-2xl widget hover:border-pink-400/30 hover:shadow-lg hover:shadow-pink-500/10 transition-all duration-300 animate-slide-up card-hover" style={{ animationDelay: `${i * 80}ms` }}>
      ...
      <section className="max-w-6xl mx-auto px-6 py-10">
        <div className="mb-6 flex items-end justify-between">
          <div>
            <h2 className="text-2xl font-bold text-white flex items-center gap-2"><Zap className="w-6 h-6 gradient-text" /> Aktualizacje</h2>
            <p className="text-slate-400 mt-1 text-sm">Najnowsze zmiany na platformie</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {UPDATES.map((u, i) => (
            <div key={i} className="group p-5 rounded-2xl widget hover:border-pink-400/30 hover:shadow-lg hover:shadow-pink-500/10 transition-all duration-300 animate-slide-up card-hover" style={{ animationDelay: `${i * 60}ms` }}>
              <div className="flex items-start gap-4">
                <div className="shrink-0">
                  <span className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${u.color}`}>{u.tag}</span>
                </div>
      ...
      <section className="max-w-6xl mx-auto px-6 py-10">
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-white flex items-center gap-2"><Star className="w-6 h-6 gradient-text" /> Wydarzenia</h2>
          <p className="text-slate-400 mt-1 text-sm">Nadchodzące wydarzenia społeczności</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {EVENTS.map((e, i) => {
            const Icon = e.icon;
            return (
              <div key={i} className="relative p-5 rounded-2xl widget overflow-hidden hover:border-pink-400/30 hover:shadow-lg hover:shadow-pink-500/10 transition-all duration-300 animate-slide-up card-hover" style={{ animationDelay: `${i * 80}ms` }}>
                <div className="absolute top-0 right-0 w-32 h-32 bg-pink-500/8 rounded-full blur-2xl" />
                <div className="relative">
                  <div className="flex items-start justify-between mb-4">
                    <div className="w-11 h-11 rounded-xl gradient-bg flex items-center justify-center glow-pink">
                      <Icon className="w-5 h-5 text-white" />
                    </div>
                    <div className="text-right">
                      <div className="text-2xl font-bold gradient-text-pk">{e.date}</div>
      ...
      <section className="max-w-6xl mx-auto px-6 py-6">
        <div className="relative rounded-2xl widget p-6 overflow-hidden">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 text-center">
            ...
            <div className="flex flex-col items-center">
              {isOnline ? (
                <>
                  <Wifi className="w-6 h-6 text-emerald-400 mb-1" />
                  <div className="flex items-center gap-1.5">
                    <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                    <span className="text-lg font-bold gradient-text-pk">Online</span>
                  </div>
                  <div className="text-xs text-slate-400 mt-0.5">Status</div>
                </>
              ) : (
                <>
                  <WifiOff className="w-6 h-6 text-rose-400 mb-1" />
                  <div className="flex items-center gap-1.5">
      ...
      <section className="max-w-6xl mx-auto px-6 py-10">
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-white flex items-center gap-2"><Sparkles className="w-6 h-6 gradient-text" /> Lista zmian</h2>
          <p className="text-slate-400 mt-1 text-sm">Historia aktualizacji platformy</p>
        </div>
...
function MockChatWidget() {
  const msgs = [
    ...
    { user: 'liyosu',   text: 'kino dzisiaj o 20?',              me: false, color: 'from-sky-500 to-indigo-500' },
